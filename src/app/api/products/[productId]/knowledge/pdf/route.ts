...
export async function POST(
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/products/${productId}/knowledge/pdf`, {
      method: "POST",
      body: formData,
    });
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ detail: errorMsg || "Failed to upload PDF" }, { status: response.status });
    }
